...
namespace RMS_Square.Areas.Regulatory.Models.DAO
{
    public class CompanyLicenseDAO : ReturnData
    {
        ...
        public bool SaveUpdate(CompanyLicenseBEL model, string userId)
        {
            try
            {
                ...
                else
                {
                    ...
                    string strCount = _dbHelper.GetValue("SELECT COUNT(1) AS RevisionNo FROM COMPANY_LICENSE WHERE IS_DELETE='N' AND COMPANY_CODE='" + EscapeSql(model.CompanyCode) + "' GROUP BY COMPANY_CODE");
        ...
        public IList<CompanyLicenseBEL> GetAllInfo(CompanyLicenseBEL model, string orderBy)
        {
            ...
            DataTable dt = _dbHelper.GetDataTable(_dbConn.SAConnStrReader(),
                string.Format(query.ToString(), model.CompanyCode, model.LicenseNo, model.SubmissionType));
            ...
            var item = (from DataRow row in dt.Rows
                        select new CompanyLicenseBEL
                        {
                            ...
                            CompanyUnitCode = row["COMPANY_UNIT_CODE"].ToString(),  // 6-digit unit
                            CompanyUnitName = row["COMPANY_UNIT_NAME"] != DBNull.Value ? row["COMPANY_UNIT_NAME"].ToString() : "",
                            CompanyCode = row["COMPANY_CODE"].ToString(),        // 4-digit parent
                            CompanyName = row["COMPANY_NAME"].ToString(),
                            Address = row["ADDRESS"].ToString(),
                            LicenseNo = row["LICENSE_NO"].ToString(),
                            CompLicenseName = row["COMP_LICENSE_NAME"].ToString(),
                            Details = row["DETAILS"].ToString(),
                            ResDept1 = row["RES_DEPT1"].ToString(),
                            ResDept2 = row["RES_DEPT2"].ToString(),
                            IsModifed = row["IS_MODIFIED"].ToString(),
