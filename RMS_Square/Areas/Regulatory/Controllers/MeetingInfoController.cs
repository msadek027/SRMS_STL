...
{
    public class MeetingInfoController : ControllerController
    {
        private FileDetailModel _fileModel = null;
        private static string _serverFilePath = string.Empty;
        private MeetingInfoDAO _dalObj = null;

        public MeetingInfoController()
        {
            _serverFilePath = Utility.GetServerPath();
            _dalObj = new MeetingInfoDAO();
        }
        //
        // GET: /Regulatory/MeetingInfo/
        [ActionAuth]
        public ActionResult frmMeetingInfo()
        {
        ...
        [HttpPost]
        public ActionResult frmMeetingInfo(MeetingInfoBEL model)
        {
            try
            {
        ...
        public ActionResult UploadFile(string refLevel1, string refLevel2, string fileSize, string refNo)
        {
            ...
            var obj = PutUploadFile(refLevel1, refLevel2, fileSize, _serverFilePath, (int)Enums.E_FormFileType.MeetingInfo, refNo);
            if (obj.Item1 == "S")
            {
        ...
        {
            var data = _dalObj.GetAllInfo(new MeetingInfoBEL(), orderBy: "DESC"); ;
            return Json(data, JsonRequestBehavior.AllowGet);
        }

        [ActionAuth]
        public ActionResult frmMeetingInfoView()
        {
            if (Session["UserID"] != null)
            {
                return View();
        ...
        [HttpPost]
        public ActionResult GetInfoByParams(MeetingInfoBEL model)
        {
