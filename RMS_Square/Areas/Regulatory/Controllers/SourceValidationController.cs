...
{
    public class SourceValidationController : ControllerController
    {
        //
        // GET: /Regulatory/SourceValidation/
        private FileDetailModel _fileModel = null;
        private static string _serverFilePath = string.Empty;
        private SourceValidationDAO _dalObj = null;

        public SourceValidationController()
        {
            _serverFilePath = Utility.GetServerPath();
            _dalObj = new SourceValidationDAO();
        }
        //
        // GET: /Regulatory/ProductReg/
        //[ActionAuth]
        public ActionResult frmSourceValidation()
        {
            if (Session["UserID"] != null)
            {
                Session["FormNameTitle"] = "Entry Source Validation";
        ...
        [HttpPost]
        public ActionResult frmSourceValidation(SourceValidationBEL model)
        {
            try
            {
                if (_dalObj.SaveUpdate(model, userId: Session["UserID"] as string))
                {
                    ...
                    return Json(new { AnnexId = _dalObj.ReturnMaxID, AnnexureNo = _dalObj.MaxID, Mode = _dalObj.IUMode, Status = "Yes", AnnexRevisionNo = _dalObj.MaxID });
                }
                return View();
            }
            catch (Exception e)
        ...
        {
            var data = _dalObj.GetAllInfo(new SourceValidationBEL(), orderBy: "DESC");
        ...
        public ActionResult UploadFile(string refLevel1, string refLevel2, string fileSize, string refNo, string sStatus)
        {
            ...
            var obj = PutUploadFile(refLevel1, refLevel2, fileSize, _serverFilePath, (int)Enums.E_FormFileType.SourceValidation, refNo);
            if (obj.Item1 == "S")
            {
                ...
                var model = new SourceValidationBEL();
                if (isSave)
                {
                    DataTable dt = _dalObj.GetFileRefno((int)Enums.E_FormFileType.SourceValidation, refLevel1, sStatus);
                    bool isAll = false;
                    if (dt.Rows.Count > 0)
                    {
        ...
        {
            _fileModel = new FileDetailModel();
            _fileModel.FileType = (int)Enums.E_FormFileType.SourceValidation;
        ...
        {
            var data = "Astras Ltd.";
            //var data = _dalObj.GetCompanyInfo(new RecipeBEL(), orderBy: "DESC"); ;
            return Json(data, JsonRequestBehavior.AllowGet);
        }

        public ActionResult frmSourceValidationRpt()
        {
            if (Session["UserID"] != null)
            {
                Session["FormNameTitle"] = "Source Validation Report";
                return View();
            }
            return Redirect(string.Format("~/Home/frmHome"));
        }
        ...
        [HttpPost]
        public ActionResult frmSourceValidationRpt(ReportModel model)
        {
            if (Session["UserId"] != null)
            {
                ...
                dt = _dalObj.GetSourceValidationForReport(model, "ASC");
                if (model.ReportType != "HTML")
                {
                    ...
                    if (model.ReportFormat == "General")
                    {
                        ...
                        {
                            rptPath = rptPath + "/SourceValidationMainRpt.rpt";
                            reportName = "SourceValidationRpt" + "_" + fromTodate;
                            model.ReportFormat = "General Report";
                        }
                    }
                    else
