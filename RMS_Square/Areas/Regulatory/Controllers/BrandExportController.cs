...
{
    public class BrandExportController : ControllerController
    {
        private FileDetailModel _fileModel = null;
        private static string _serverFilePath = string.Empty;
        private BrandExportDAO _dalObj = null;

        public BrandExportController()
        {
            _serverFilePath = Utility.GetServerPath();
            _dalObj = new BrandExportDAO();
        }

        // GET: /Regulatory/BrandExport/
        [ActionAuth]
        public ActionResult frmBrandExport()
        {
            if (Session["UserID"] != null)
            {
                Session["FormNameTitle"] = "Entry Brand Name (Export)";
                return View();
            }
        ...
        public ActionResult frmBrandExport(BrandExportBEL model)
        {
            try
            {
        ...
        {
            var data = _dalObj.GetAll(new BrandExportBEL(), orderBy: "DESC");
        ...
        public ActionResult GetFileByRefId(string refLevel1, string refLevel2)
        {
            ...
            _fileModel.FileType = (int)Enums.E_FormFileType.BrandExport;
            _fileModel.RefLevel1 = refLevel1;
            _fileModel.RefLevel2 = refLevel2;
            return Json(new { dataFile = GetFileByParameters(_fileModel).OrderBy(o => o.FileID) }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult UploadFile(string refLevel1, string refLevel2, string fileSize, string refNo)
        {
            var obj = PutUploadFile(refLevel1, refLevel2, fileSize, _serverFilePath, (int)Enums.E_FormFileType.BrandExport, refNo);
            if (obj.Item1 == "S")
            {
                _fileModel = new FileDetailModel();
        ...
        [ActionAuth]
        public ActionResult frmBrandExportRpt()
        {
            if (Session["UserID"] != null)
            {
                Session["FormNameTitle"] = "Brand Name (Export) Report";
                return View();
            }
            return Redirect(string.Format("~/Home/frmHome"));
        }
        [HttpPost]
        public ActionResult frmBrandExportRpt(ReportModel model)
        {
            if (Session["UserId"] != null)
            {
                ...
                {
                    rptPath = rptPath + "/BrandExportRpt.rpt";
                    string reportName = "BrandExportRpt" + "_" + fromTodate;
